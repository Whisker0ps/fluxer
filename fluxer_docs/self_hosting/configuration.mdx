...
<Note>
When self-hosting: to have the web app use your domain for static assets (scripts, fonts, favicons) and avoid CSP errors, create `config/config.json` with `domain.base_domain` set to your public host **before** building the Docker image. The app build reads this via `FLUXER_CONFIG` and bakes the static CDN URL into the built app.
</Note>
